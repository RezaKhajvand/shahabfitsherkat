...
import 'package:shahabfit/Features/Activities/Data/BasketActivityCollection/updateBasketDataSource.dart';
...
class City {
  ...
  City({required this.country, required this.name});
}
                ...
                onPressed: () async {
                            ...
                            children: [
                              ...
                              SizedBox(
                                ...
                                child: ElevatedButton(
                                    ...
                                    child: const Text('دانلود فایل برنامه')),
                              )
                            ],
                          ),
                        ),
                  ...
                  if (isClose) {
                    try {
                      ...
                      for (var i = 0; i < 7; i++) {
                        if (pdfItems
                            .where((element) => element.dayOfWeek == i)
                            .isNotEmpty) {
                          ++dayCount;
                        }
                      }
                      ...
                      await closeBasket(
                          basketId: widget.basketInputs.basketId,
        ...
        body: BlocBuilder<BasketBloc, BasketState>(
          builder: (context, state) {
            if (state is BasketLoaded) {
              pdfItems = state.basketActivity.items;
              var basketActivity = state.basketActivity.items
                      ...
                      child: Column(
                        children: [
                          ...
                          ListView.separated(
                            ...
                            shrinkWrap: true,
                            itemCount: activitySet.length + 1,
                            itemBuilder: (context, setindex) {
                              if (setindex == activitySet.length) {
                                    ...
                                    onPressed: () async {
                                      BlocProvider.of<BasketBloc>(context).add(
                                          UpdateBasketActivityEvent(
                                              basketActivityId:
                                                  basketActivity[index].id,
                                              activitySet: basketActivity[index]
                                                  .activitySet
                                                ..add([3, 10])));
                                    },
