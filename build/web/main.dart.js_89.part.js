((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_89",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var A,B={F9:function F9(d){this.a=d},aks:function aks(d){this.a=d}},C
A=c[0]
B=a.updateHolder(c[63],B)
C=c[172]
B.F9.prototype={
I(d){var x=null
return A.de(x,x,x,C.Q8,x,x,new B.aks(d),x,x,x,x)}}
var z=a.updateTypes([])
B.aks.prototype={
$0(){var x=0,w=A.w(y.b),v,u=this
var $async$$0=A.r(function(d,e){if(d===1)return A.t(e,w)
while(true)switch(x){case 0:v=A.cg(u.a).nD("/main",null)
x=1
break
case 1:return A.u(v,w)}})
return A.v($async$$0,w)},
$S:8};(function inheritance(){var x=a.inherit
x(B.F9,A.a8)
x(B.aks,A.cb)})()
A.bd(b.typeUniverse,JSON.parse('{"F9":{"a8":[],"d":[]}}'))
var y={b:A.F("~")};(function constants(){C.PW=new A.bW(63477,"MaterialIcons",null,!1)
C.Q8=new A.cJ(C.PW,null,null,null,null)})()};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_89",e:"endPart",h:b})})($__dart_deferred_initializers__,"AKQ4sCKkFcEc8AHKxjcKgUky9L4=");