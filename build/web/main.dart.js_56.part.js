((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_56",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var A,C,D,E,B={
bdA(){return new B.r2(null)},
r2:function r2(d){this.a=d},
a8i:function a8i(){this.c=this.a=null},
aM_:function aM_(d){this.a=d}},F
A=c[0]
C=c[2]
D=c[61]
E=c[108]
B=a.updateHolder(c[6],B)
F=c[33]
B.r2.prototype={
a8(){return new B.a8i()}}
B.a8i.prototype={
av(){this.aR()
this.A4()},
A4(){var x=0,w=A.w(y.b),v=this
var $async$A4=A.r(function(d,e){if(d===1)return A.t(e,w)
while(true)switch(x){case 0:x=2
return A.K(A.pG(C.fY,new B.aM_(v),y.f),$async$A4)
case 2:return A.u(null,w)}})
return A.v($async$A4,w)},
I(d){var x=null,w="images/splash.png",v=A.bh(d,x,y.h).w.a.a/1.1
return A.ej(x,C.o,A.dl(new A.el(C.T,x,C.bo,C.J,A.a([D.hS(w,E.dc,x,v),F.b_W(C.u,D.hS(w,E.dc,x,v),C.f,C.fY)],y.e),x),x,x),x,!1,x,x)}}
var z=a.updateTypes([])
B.aM_.prototype={
$0(){var x=this.a.c
x.toString
return A.cg(x).oX("/main",null,y.a)},
$S:0};(function inheritance(){var x=a.inherit
x(B.r2,A.I)
x(B.a8i,A.O)
x(B.aM_,A.cb)})()
A.bd(b.typeUniverse,JSON.parse('{"r2":{"I":[],"d":[]},"a8i":{"O":["r2"]}}'))
var y={e:A.F("m<d>"),h:A.F("dP"),b:A.F("@"),a:A.F("q?"),f:A.F("~")}};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_56",e:"endPart",h:b})})($__dart_deferred_initializers__,"lN4yFddnvicvsKF5UEHRTLEfC2k=");